...
{
    internal class TupleElementWorker
    {
        public List<(ElementType type, int level, string text)> GetElements(
            string[] lines)
        {
            var elements = new List<(ElementType, int, string)>();
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                if (IsHeader(line))
                {
                    var elem = (ElementType.Header, GetHeaderLevel(line), GetHeaderText(line));
                    elements.Add(elem);
                }
                else
                {
                    var elem = (ElementType.Line, GetLineLevel(line), GetLineText(line));
                    elements.Add(elem);
                }
            }
