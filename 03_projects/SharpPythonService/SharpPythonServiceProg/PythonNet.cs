...
namespace PythonNetEngine
{
    public class PythonNet : IPythonEngine
    {
        ...
        private void SetPythonPaths()
        {
            ...
            var pythonInstallDir = "C:/Users/Pawel/AppData/Local/Programs/Python/Python39";
            var pyInstallDir = Environment.ExpandEnvironmentVariables(pythonInstallDir);
            var pyDll = CheckPythonInstalled(pyInstallDir);
        ...
        private string CheckPythonInstalled(string installDir)
        {
            ...
            var pyDllList = Directory.GetFiles(installDir)
                .Where(s => Path.GetFileName(s).StartsWith("python3") && Path.GetFileName(s).EndsWith(".dll"))
                .ToList();
            var pyDll2 = pyDllList.Last();
            if (pyDll == null)
            {
